...
namespace SV
{
    ...
    public partial struct DropOnDeathSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            state.EntityManager.RemoveComponent(m_deadQuery, new TypePack<DropOnDeathCleanup, DropOnDeathTransformCleanup>());

            var newEntities = m_newQuery.ToEntityArray(state.WorldUpdateAllocator);
            state.EntityManager.AddComponent(m_newQuery, new TypePack<DropOnDeathCleanup, DropOnDeathTransformCleanup>());
            foreach (var entity in newEntities)
            {
                var prefabs = state.EntityManager.GetBuffer<DropOnDeath>(entity, true).Reinterpret<DropOnDeathCleanup>().AsNativeArray();
                state.EntityManager.GetBuffer<DropOnDeathCleanup>(entity, false).AddRange(prefabs);
                state.EntityManager.SetComponentData(entity, new DropOnDeathTransformCleanup { transform = TransformQvvs.identity });
            }
        }
    }
}
